...
To maintain the state of a successful login instead of forcing a user to relogin everytime they move to a new path which requires authentication.

2. What does bcrypt do to help us store passwords in a secure manner.
We can use bcrypt to encrypt the password by hashing it and then storing the hash that matches with that password instead of the actual password.

3. What does bcrypt do to slow down attackers?
By hashing the password we store nothing on our end about the user's actual password information. Even if the database is compromised we maintain the security of the user's passwords. 
This forces attackers to do a brute force attack and check every possible combination of a password against our stored hashes.

4. What are the three parts of the JSON Web Token?
The header, payload and signature. Each seperated by a dot.
